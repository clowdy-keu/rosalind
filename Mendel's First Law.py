...
def mendel_first_law(k, m, n):
    dominant_count = 0
    all_count = (k + m + n) * (k + m + n - 1) * 4
    for homo_dom in range(k):
        dominant_count += (k + m + n - 1) * 4
    for homo_rec in range(n):
        dominant_count += k * 4 + m * 2
    for hetero in range(m):
        dominant_count += k * 4 + (m-1) * 3 + n * 2
    return dominant_count/all_count


print(mendel_first_law(29, 28, 17))
